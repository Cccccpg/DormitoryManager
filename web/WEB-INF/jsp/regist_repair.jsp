...
<body>
<form action="${pageContext.request.contextPath}/repair/addLogin" method="post" name="myform">
    <table class="table" style="width: 100%;text-align: center;">
        ...
        <tr>
            <td><label for="dorm1">选择校区</label></td>
            <td>
            ...
            <td><label for="dorm2">选择楼栋</label></td>
            <td>
        ...
        <tr>
            <td><label for="dorm3">选择宿舍号</label></td>
            <td>
        ...
        <tr>
            <td><label for="repair_reason">报修原因</label></td>
            <td>
                <input type="text" class="form-control" value="" id="repair_reason" name="repair_reason" />
            </td>
        </tr>
        <tr>
            <td colspan="4">
                <input type="button" id="sub-btn" class="btn f30 btn-primary radius10 p2 w50" value="提交登记" />
                <a href="${pageContext.request.contextPath}/repair/findAll" target="_self" class="btn btn-default">返回列表</a>
            </td>
        </tr>
        </tbody>
    </table>
...
<script>
    $("#sub-btn").click(function () {
        ...
        var d3 = $("#dorm3").val();
        var place = d1+"  "+d2+"  "+d3;
        var dorm_id = place.trim();
        var repair_reason = $("#repair_reason").val().trim();

        if (name == null || sno == null || phone == null || dorm_id == null || repair_reason == null
            || name.length == 0 || sno.length == 0 || phone.length == 0 || dorm_id.length == 0 || repair_reason.length == 0) {
            layer.msg('字段不能为空');
            return false;
        }
        ...
        $.ajax({
            url: "${pageContext.request.contextPath}/repair/addLogin",//要请求的服务器url
